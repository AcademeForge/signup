...
(function(){
  ...
  function genCode(){
    var s='';
    for(var i=0;i<6;i++) s+=CHARS[rnd(0,CHARS.length-1)];
    return s;
  }
  ...
  function drawCaptcha(code){
    ...
    if(dark){bg.addColorStop(0,'#1f2937');bg.addColorStop(1,'#111827');}
    else{bg.addColorStop(0,'#fff5f5');bg.addColorStop(1,'#fff0f6');}
    ctx.fillStyle=bg;
    ...
    for(var l=0;l<5;l++){
      ctx.beginPath();
      ctx.moveTo(rnd(0,W*0.3),rnd(0,H));
      ctx.bezierCurveTo(
        rnd(W*0.2,W*0.5),rnd(0,H),
        rnd(W*0.5,W*0.8),rnd(0,H),
        rnd(W*0.7,W),rnd(0,H)
      );
      ctx.strokeStyle=dark
        ?'rgba(251,113,133,'+(0.12+Math.random()*0.15)+')'
        :'rgba(251,113,133,'+(0.18+Math.random()*0.18)+')';
      ctx.lineWidth=1.2+Math.random()*1;
      ctx.stroke();
    }

    var colors=dark
      ?['#fb7185','#f9a8d4','#fbbf24','#a5f3fc','#86efac','#c4b5fd']
      :['#e11d48','#9333ea','#0284c7','#059669','#d97706','#7c3aed'];
    var cellW=W/6;
    for(var i=0;i<6;i++){
      ctx.save();
      ctx.translate(cellW*i+cellW/2, H/2);
      ctx.rotate((Math.random()-0.5)*0.45);
      var size=rnd(26,34);
      ctx.font='bold '+size+'px "Plus Jakarta Sans",Arial,sans-serif';
      ctx.textAlign='center';
      ctx.textBaseline='middle';
      var vy=(Math.random()-0.5)*8;
      ctx.strokeStyle=dark?'rgba(0,0,0,0.5)':'rgba(255,255,255,0.85)';
      ctx.lineWidth=3;
      ctx.strokeText(code[i],0,vy);
      ctx.fillStyle=colors[i%colors.length];
      ctx.fillText(code[i],0,vy);
      ctx.restore();
    }
  }
  ...
  function newCaptcha(){
    ...
    if(inp){inp.value='';inp.classList.remove('cap-err');}
    setCapMsg('','');
  }

  function setCapMsg(type,text){
    var el=document.getElementById('captcha-msg');
    if(!el) return;
    el.className=type;
    el.textContent=text;
  }
  ...
  function unlockCaptcha(){
    ...
    ov.style.opacity='0';
    document.body.classList.remove('cap-locked');
  ...
  function shakeInput(){
    var inp=document.getElementById('captcha-input');
    if(!inp) return;
    inp.classList.remove('cap-err');
    void inp.offsetWidth;
    inp.classList.add('cap-err');
  }
  ...
  function checkCaptcha(){
    ...
    if(val.length<6){setCapMsg('cap-msg-err','Please enter all 6 characters.');shakeInput();return;}
    if(val===_code){
      setCapMsg('cap-msg-ok','✓ Access granted!');
      setTimeout(unlockCaptcha,320);
    } else {
      shakeInput();
      setCapMsg('cap-msg-err','Incorrect — here\'s a new code.');
      inp.value='';
  ...
  function initCaptcha(){
    ...
    if(btn) btn.addEventListener('click',checkCaptcha);
    var ref=document.getElementById('cap-refresh-icon');
    if(ref) ref.addEventListener('click',newCaptcha);
    var inp=document.getElementById('captcha-input');
    if(inp) inp.addEventListener('keydown',function(e){if(e.key==='Enter')checkCaptcha();});

    var obs=new MutationObserver(function(){drawCaptcha(_code);});
    obs.observe(document.documentElement,{attributes:true,attributeFilter:['data-theme']});

    document.body.classList.add('cap-locked');
    if(document.fonts&&document.fonts.ready){
      document.fonts.ready.then(newCaptcha);
    } else {
...
(function(){
  ...
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded', initFooterAcc);
  } else {
    initFooterAcc();
  }
})();
